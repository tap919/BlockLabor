...
if ($Backend -eq 'podman') {
    ...
    Write-Host "Initializing Podman machine 'overcoatVM'..."
    try {
        & podman machine init overcoatVM
        Write-Host "Podman machine 'overcoatVM' initialized or already exists."
    } catch {
    ...
    Write-Host "Starting Podman machine 'overcoatVM'..."
    try {
        & podman machine start overcoatVM -q
    ...
    Write-Host "Setting default Podman connection to 'overcoatVM'..."
    try {
        & podman system connection default overcoatVM
