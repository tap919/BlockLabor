# ADR-005: RVF Cognitive Container Format
...
We adopt RVF as the universal format for vector data with the following segment architecture:
...
| KERNEL | 0x0E | RVKN | Native unikernel (self-booting) |
| EBPF | 0x0F | RVBF | eBPF programs |
...
### 5.2 Tiered Compression

```
HOT_SEG:      None (immediate access)
...
crates/rvf/src/
    ...
    header.rs           # RvfHeader, SegmentHeader
    reader.rs           # RvfReader (streaming, mmap)
    writer.rs           # RvfWriter (builder pattern)
    manifest.rs         # Manifest JSON schema
    segments/
        vectors.rs      # VECTORS_SEG
        hnsw.rs         # HNSW_SEG
        crypto.rs       # CRYPTO_SEG
        wasm.rs         # WASM_SEG
        witness.rs      # WITNESS_SEG
    compression.rs      # Compression dispatch
    signature.rs        # ML-DSA signing/verification
```
...
| Component | Size |
|-----------|------|
| Vectors (f32, LZ4) | ~850 MB |
| HNSW index (Zstd) | ~45 MB |
| Manifest | ~2 KB |
| Crypto segment | ~8 KB |
| WASM kernel | ~6 KB |
| **Total** | **~895 MB** |

Overhead vs raw vectors: 895 / (1M * 384 * 4) = **58% of raw** (due to compression)

## 8. Related Decisions

- **ADR-029-rvf-canonical-format**: Detailed wire format specification
- **ADR-030-rvf-cognitive-container**: Self-booting unikernel extension
- **ADR-032-rvf-wasm-integration**: WASM runtime integration
- **ADR-005-wasm-runtime-integration**: Runtime selection (Wasmtime)
