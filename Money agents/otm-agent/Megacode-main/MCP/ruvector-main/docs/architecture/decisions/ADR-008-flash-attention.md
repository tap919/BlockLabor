# ADR-008: Flash Attention Implementation Choices
...
| **Supersedes** | - |
| **Related** | ADR-001-core-simd-strategy, ADR-006-sona-adaptation |
...
|--------|-------------------|-----------------|
| Memory for attention matrix | O(N^2) | O(N) (output only) |
| HBM reads/writes | O(N^2) | O(N^2 / M) where M = SRAM size |
        ...
        for h in 0..heads {
            for j in 0..num_blocks_k {
                ...
                for i in 0..num_blocks_q {
                    ...
                    let l_old_scaled = lse_block.exp() * &scale_old;
...
/// NEON-optimized GEMM for small blocks
#[cfg(target_arch = "aarch64")]
unsafe fn neon_gemm(
    a: &[f32],      // [M, K]
    b: &[f32],      // [N, K] (note: B is transposed)
    c: &mut [f32],  // [M, N]
    m: usize,
    n: usize,
    k: usize,
    ...
    for i in 0..m {
        for j in (0..n).step_by(4) {
            ...
            for kk in (0..k).step_by(4) {
                ...
                for jj in 0..4.min(n - j) {
                    let b_vec = vld1q_f32(b.as_ptr().add((j + jj) * k + kk));
                    sum = vfmaq_f32(sum, a_vec, b_vec);
                }
            }

            // Horizontal sum and store
            let result = vaddvq_f32(sum) * scale;
            c[i * n + j] = result;
        }
    }
}
...
| Causal | Lower triangular | Autoregressive generation |
...
pub enum AttentionMask {
    ...
    Custom { blocks: Vec<(usize, usize, usize, usize)> },
}
...
impl AttentionMask {
        ...
        -> Vec<(usize, usize)>
    {
        match self {
            AttentionMask::Dense => {
                let num_q = (seq_q + block_size - 1) / block_size;
                let num_k = (seq_k + block_size - 1) / block_size;
                (0..num_q).flat_map(|i| (0..num_k).map(move |j| (i, j))).collect()
            }
            AttentionMask::Causal => {
                let num_blocks = (seq_q.max(seq_k) + block_size - 1) / block_size;
                (0..num_blocks).flat_map(|i| (0..=i).map(move |j| (i, j))).collect()
            }
            AttentionMask::SlidingWindow { window_size } => {
                let blocks_in_window = (window_size + block_size - 1) / block_size;
                let num_blocks = (seq_q.max(seq_k) + block_size - 1) / block_size;
                (0..num_blocks).flat_map(|i| {
                    let start = i.saturating_sub(blocks_in_window);
                    (start..=i).map(move |j| (i, j))
                }).collect()
            }
            // ... other patterns
        }
...
5. **SONA Integration**: Adaptive ef maps naturally to adaptive block sizes
...
|-------------|--------------|
| Standard attention | O(N^2) memory, doesn't scale |
| Sparse attention | Approximation, misses patterns |
| Linear attention | Poor quality, different semantics |
| Ring attention | More complex, similar benefits |
| Paged attention | Complementary, used for KV cache |

## 6. Related Decisions

- **ADR-001-core-simd-strategy**: SIMD for efficient block operations
- **ADR-006-sona-adaptation**: SONA adapts attention parameters dynamically
- **ADR-005-cross-platform-bindings**: Flash attention available in WASM
