...
type DevMode = "beginner" | "intermediate" | "expert";
...
const DOMAIN_DESCRIPTIONS: Record<Exclude<Domain, "custom">, string> = {
  coding:
    "Software engineering, algorithms, architecture, and full-stack development",
  biotech:
    "Molecular biology, genomics, drug development, and bioinformatics",
  fintech:
    "Financial technology, quantitative finance, and capital markets",
  marketing:
    "Growth marketing, brand strategy, analytics, and martech",
  gamedev:
    "Game engines, rendering, gameplay programming, and game design",
  devops:
    "CI/CD, containers, Kubernetes, IaC, observability, and SRE",
...
const FORMAT_DESCRIPTIONS: Record<BreakdownFormat, string> = {
  ...
  walkthrough:
    "Step-by-step code walkthrough for understanding unfamiliar code",
...
const DEV_MODE_DESCRIPTIONS: Record<DevMode, string> = {
  beginner:
    "New to the topic. Simple language, analogies, no jargon, step-by-step.",
  intermediate:
    "Solid working knowledge. Standard terminology, nuance, best practices.",
  expert:
    "Highly experienced. Dense, technical, edge cases, production-grade. Peer-level.",
};
...
function getDomainContext(
  ...
  return DOMAIN_CONTEXTS[domain as Exclude<Domain, "custom">];
}
...
function buildBreakdownPrompt(
  ...
  const domainLabel =
    domain === "custom" ? (customDomainName || "Custom") : domain;
...
function buildBookToGuidePrompt(
  text: string,
  domain: Domain,
  filename?: string
): PromptResult {
...
const DevModeEnum = z.enum(["beginner", "intermediate", "expert"]);
...
const BreakdownSchema = z.object({
  ...
  customDomainPrompt: z.string().optional(),
});
...
const BookToGuideSchema = z.object({
  text: z.string(),
  domain: DomainEnum.default("coding"),
...
const server = new Server(
  { name: "bobby-breakdown-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
    ...
    tools: [
      {
        name: "breakdown",
        description:
          "Generate a Bobby Breakdown prompt for AI-powered concept explanation. Returns system prompt and messages for the LLM to process. Supports 8 domains, 14 formats, and 3 dev modes.",
        inputSchema: {
          ...
          properties: {
            ...
            customDomainName: {
              type: "string",
              description:
                "Name for custom domain (only used when domain is 'custom')",
            },
            customDomainPrompt: {
              type: "string",
              description:
                "System prompt for custom domain (only used when domain is 'custom')",
            },
          },
          required: ["text"],
        },
      },
      {
        name: "compare",
        description:
          "Generate a Bobby Breakdown comparison prompt. Compares two concepts side by side with differences, similarities, and when-to-use guidance.",
        inputSchema: {
          type: "object" as const,
          properties: {
            conceptA: {
      ...
      },
      {
        name: "fastscan",
        description:
          "Generate a Bobby Breakdown Fast Scan prompt. Performs rapid code triage giving instant situational awareness about pasted code.",
        inputSchema: {
          type: "object" as const,
          properties: {
            code: {
      ...
      },
      {
        name: "book_to_guide",
        description:
          "Generate a Bobby Breakdown Book-to-LLM-Guide prompt. Transforms a book or document into a compressed, information-dense reference optimized for AI agents.",
        inputSchema: {
          type: "object" as const,
          properties: {
            text: {
              type: "string",
              description:
                "The book or document text to convert into an LLM Guide",
            },
            domain: {
              type: "string",
              enum: [
      ...
      },
      {
        name: "followup",
        description:
          "Generate a Bobby Breakdown follow-up prompt. For asking follow-up questions about a previous breakdown, with optional conversation history.",
        inputSchema: {
          type: "object" as const,
          properties: {
            question: {
      ...
      },
      {
        name: "list_domains",
        description:
          "List all 8 built-in Bobby Breakdown knowledge domains with descriptions.",
        inputSchema: {
          type: "object" as const,
          properties: {},
        },
      },
      {
        name: "list_formats",
        description:
          "List all 14 Bobby Breakdown output formats with descriptions.",
        inputSchema: {
          type: "object" as const,
          properties: {},
        },
      },
      {
        name: "list_dev_modes",
        description:
          "List the 3 Bobby Breakdown developer experience levels with descriptions.",
        inputSchema: {
          type: "object" as const,
          properties: {},
        },
    ...
    switch (name) {
      case "breakdown": {
        ...
        const result = buildBreakdownPrompt(
          ...
          parsed.format,
          parsed.devMode,
          parsed.customDomainName,
          parsed.customDomainPrompt
        );
        return {
          content: [
      ...
      case "fastscan": {
        ...
        const result = buildFastScanPrompt(
          parsed.code,
          parsed.domain,
          parsed.devMode,
          parsed.customDomainName,
      ...
      case "book_to_guide": {
        ...
        const result = buildBookToGuidePrompt(
          parsed.text,
          parsed.domain,
          parsed.filename
        );
      ...
      case "list_domains": {
        const domains = Object.entries(DOMAIN_DESCRIPTIONS).map(
          ([key, description]) => ({ name: key, description })
        );
        return {
          content: [
            {
      ...
      case "list_formats": {
        const formats = Object.entries(FORMAT_DESCRIPTIONS).map(
          ([key, description]) => ({ name: key, description })
        );
        return {
          content: [
      ...
      case "list_dev_modes": {
        const devModes = Object.entries(DEV_MODE_DESCRIPTIONS).map(
          ([key, description]) => ({ name: key, description })
        );
        return {
          content: [
