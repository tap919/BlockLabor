...
export interface CreatorPageEvents {
  ...
  /** User clicked "Refine with Vibe Coder" — inject prompt into chat. */
...
export class VibeCreatorPage {
  ...
  private buildDOM(): HTMLElement {
    ...
    tweakPanel.innerHTML = `
      <div class="tweak-empty-state">
        <div style="font-size:28px; opacity:0.3; margin-bottom:8px;">◈</div>
        <div style="font-size:13px; color:var(--text-dim);">Select a preset to customize</div>
      </div>
    `;
  ...
  private renderTweakPanel(): void {
    ...
    if (!this.selectedPreset) {
      panel.innerHTML = `
        <div class="tweak-empty-state">
          <div style="font-size:28px; opacity:0.3; margin-bottom:8px;">◈</div>
          <div style="font-size:13px; color:var(--text-dim);">Select a preset from the grid to customize its parameters</div>
          <div style="font-size:11px; color:var(--text-dim); margin-top:8px;">Use the search or browse by category</div>
        </div>
      `;
      return;
    }
    ...
    if (count > 0) {
      ...
      startBtn.addEventListener('click', () => {
        ...
        for (const cat of PRESET_CATEGORIES) {
          ...
          if (p) lines.push(`• ${cat.label}: ${p.title}`);
        }
