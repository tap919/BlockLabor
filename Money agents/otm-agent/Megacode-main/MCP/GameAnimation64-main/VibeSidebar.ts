...
import { Viewport3D, RenderMode, BudgetWarning } from './Viewport3D.js';
import { GameComponentTemplate, getGroupedComponents } from './VibeGameComponents.js';

// ─── Event map ────────────────────────────────────────────────────────────────

export interface SidebarEvents {
  entitySelect:    (entityName: string) => void;
  renderModeChange:(mode: RenderMode)   => void;
  gridToggle:      (visible: boolean)   => void;
  cartoonToggle:   (enabled: boolean)   => void;
  applyComponent:  (comp: GameComponentTemplate) => void;
}
...
// ─── Budget snapshot (fed from Viewport3D callbacks) ──────────────────────────

export interface BudgetSnapshot {
  tris:      { used: number; max: number };
  verts:     { used: number; max: number };
...
export class VibeSidebar {
  ...
  private handlers:      Partial<SidebarEvents> = {};
  private entities:      SidebarEntity[]        = [];
  ...
  private budgetTrisFill!: HTMLElement;
  private budgetVertFill!: HTMLElement;
  private budgetRamFill!:  HTMLElement;
  ...
  on<K extends keyof SidebarEvents>(event: K, handler: SidebarEvents[K]): this {
    this.handlers[event] = handler as any;
    return this;
  }

  /** Replace the entity list and re-render the tree. */
  setEntities(entities: SidebarEntity[]): void {
  ...
  syncRenderMode(mode: RenderMode): void {
  ...
  private buildTabs(root: HTMLElement): void {
    ...
    const defs: Array<{ id: string; icon: string; label: string }> = [
  ...
  private renderComponentList(root: HTMLElement): void {
  ...
  private buildRenderModes(root: HTMLElement): void {
    const container = root.querySelector('#sidebar-render-modes')!;
    const modes: Array<{ mode: RenderMode; label: string; icon: string }> = [
  ...
  private cacheRefs(root: HTMLElement): void {
  ...
  private wireEvents(root: HTMLElement): void {
    // Debounce utility for snappy button response
    const debounceTimers = new Map<string, ReturnType<typeof setTimeout>>();
    const debounce = (key: string, fn: () => void, ms = 80) => {
      const timer = debounceTimers.get(key);
      if (timer) clearTimeout(timer);
      debounceTimers.set(key, setTimeout(fn, ms));
    };

    // Grid toggle - debounced for rapid clicks
    root.querySelector('#toggle-grid')?.addEventListener('click', () => {
      debounce('grid', () => {
        ...
        if (this.gridVisible) {
          ...
          (badge as HTMLElement).style.cssText = 'margin-left:auto; color:var(--neon-green); border-color:rgba(57,255,20,0.35); background:rgba(57,255,20,0.10);';
        } else {
          badge.textContent = 'OFF';
          (badge as HTMLElement).style.cssText = 'margin-left:auto;';
        }
        this.handlers.gridToggle?.(this.gridVisible);
      });
    });

    // Cartoon toggle - debounced for rapid clicks
    root.querySelector('#toggle-cartoon')?.addEventListener('click', () => {
      debounce('cartoon', () => {
        ...
        if (this.cartoonEnabled) {
          badge.textContent = 'ON';
          (badge as HTMLElement).style.cssText = 'margin-left:auto; color:var(--neon-cyan); border-color:rgba(0,212,255,0.35); background:rgba(0,212,255,0.10);';
        } else {
          badge.textContent = 'OFF';
          (badge as HTMLElement).style.cssText = 'margin-left:auto;';
        }
  ...
  private renderEntityTree(): void {
    ...
    const renderNode = (entity: SidebarEntity, depth: number): void => {
      const item = document.createElement('div');
  ...
  private renderBudget(): void {
    ...
    const setBar = (fill: HTMLElement, val: HTMLElement, pct: number, used: number, suffix: string) => {
