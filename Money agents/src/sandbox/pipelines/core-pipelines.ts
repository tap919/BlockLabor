/**
 * Core Financial Pipelines
 * Implements the five core pipeline families:
 * 1. Lead-to-Revenue
 * 2. Quote-to-Cash
...
import { financialSandbox, DecisionObject, EvidenceRef } from '../financial-sandbox';
...
export class LeadToRevenuePipeline {
   ...
   * Process a new lead
   */
  async processLead(lead: LeadData): Promise<PipelineResult> {
    const errors: string[] = [];
    ...
    try {
      ...
      // Step 3: Score intent and value
      const score = this.scoreLead(enrichedLead);

      // Step 4: Create decision object
      ...
      if (decision.status === 'approved' || !decision.approvalRequired) {
        ...
        // Step 6: Draft outreach if high value
   ...
   * Normalize lead data
   */
  private normalizeLead(lead: LeadData): LeadData {
    return {
      ...lead,
...
export class QuoteToCashPipeline {
  ...
  /**
   * Generate and process a quote
   */
  async processQuote(quote: QuoteData): Promise<PipelineResult> {
    const errors: string[] = [];
    const executedActions: string[] = [];
    ...
    try {
      ...
      // Step 2: Check pricing policies
      const policyCheck = this.checkPricingPolicy(quote, totals);
      ...
      const decision = await financialSandbox.createDecision({
        ...
        reasoningSummary: `Quote for $${totals.total} with ${quote.discount || 0}% discount`,
        evidenceRefs: [
          { source: 'quote_form', recordId: quote.leadId, field: 'leadId', value: quote.leadId },
        ],
        executionPlan: {
          steps: [
            { order: 1, verb: 'create_stripe_invoice', target: 'stripe', params: { quote, totals }, rollback: 'void_stripe_invoice' },
          ],
          dependencies: [],
          timeout: 45000,
        },
      ...
      return {
        ...
        executedActions,
        errors,
        data: { quote, totals, decision, policyCheck },
      };
   ...
   */
  private checkPricingPolicy(quote: QuoteData, totals: { total: number }): {
    valid: boolean;
    violations: string[];
    ...
    if (quote.discount && quote.discount > 20) {
      violations.push(`Discount of ${quote.discount}% exceeds 20% threshold`);
...
export class AccountsReceivablePipeline {
   ...
   */
  async processFollowUp(invoice: InvoiceData): Promise<PipelineResult> {
    const errors: string[] = [];
    const executedActions: string[] = [];

    try {
      // Step 1: Analyze invoice status
      const analysis = this.analyzeInvoice(invoice);

      // Step 2: Generate follow-up plan
      const followUpPlan = this.generateFollowUpPlan(invoice, analysis);
      ...
      const decision = await financialSandbox.createDecision({
        ...
        entity: {
          ...
          data: { invoice, analysis },
        },
        reasoningSummary: `Invoice ${invoice.id} is ${analysis.daysOverdue} days overdue. ${followUpPlan.action}`,
        evidenceRefs: [
          { source: 'invoice_system', reference: invoice.id || 'unknown', type: 'input' },
          { source: 'payment_history', reference: invoice.customerId, type: 'derived' },
        ],
        executionPlan: {
      ...
      return {
        ...
        executedActions,
        errors,
        data: { invoice, analysis, followUpPlan, decision },
      };
  ...
  private generateFollowUpPlan(invoice: InvoiceData, analysis: ReturnType<typeof this.analyzeInvoice>): {
    action: string;
    steps: Array<{ verb: string; target: string; payload: any }>;
  } {
    const steps: Array<{ verb: string; target: string; payload: any }> = [];
...
export class ExpenseVendorPipeline {
   ...
   */
  async processVendorInvoice(vendorInvoice: VendorInvoice): Promise<PipelineResult> {
    const errors: string[] = [];
    const executedActions: string[] = [];
    ...
    try {
      ...
      const approvalRequired = this.requiresApproval(vendorInvoice);
   ...
   */
  private async checkDuplicates(invoice: VendorInvoice): Promise<{ isDuplicate: boolean; matchingInvoice?: any }> {
   ...
   */
  private requiresApproval(invoice: VendorInvoice): boolean {
    // Approval required for large amounts
    if (invoice.amount > 5000) return true;

    // Approval required for certain categories
...
export class ComplianceAnomalyPipeline {
   ...
   */
  async processAnomaly(anomaly: TransactionAnomaly): Promise<PipelineResult> {
    const errors: string[] = [];
    const executedActions: string[] = [];
